<template>
  <div
    class="hidden md:flex min-w-[38rem] bg-radial-[at_100%_0%] from-primary-yellow/40 to-black to-80% text-white items-end justify-center rounded-3xl pb-[8rem] px-[6.25rem] shadow-md">
    <div class="flex flex-col items-center text-center max-w-md px-6">
      <NuxtLink to="/"><img src="/assets/images/logo_light.png" alt="Logo" class="w-48 mb-8" /></NuxtLink>
      <p class="text-2xl font-semibold mb-8">Easy. <span class="text-yellow-400">Fast.</span> Online.</p>
      <p class="text-white">
        Your one stop solution for getting in touch with your local tattoo artists,
        get inspirations, calculate costs.
      </p>
    </div>
  </div>
</template>